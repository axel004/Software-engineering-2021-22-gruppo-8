...
public class AddVarCommand implements Command{
    ...
    //la funzione execute controlla che l'operazione richiesta dall'utente sia corretta e quindi chiama la funzione sumVariable
    ...
    public boolean execute(String text) throws VariableException {
        ...
        if (var.getValue(opval[1]) != null && stack.size() >= 1) { //controllo l'esistenza della chiave e l'esistenza di almeno un valore nello stack
            stack.insertAux(var.getValue(opval[1])); //metodo ausiliario per il metodo undo che prende il valore presente nella variabil
